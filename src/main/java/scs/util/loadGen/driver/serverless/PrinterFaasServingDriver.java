...
public class PrinterFaasServingDriver extends AbstractJobDriver{
    /**
     * Singleton code block
     */
    private static PrinterFaasServingDriver driver=null;
    private SSHTool tool = new SSHTool("192.168.1.4", "root", "wnlof309b507", StandardCharsets.UTF_8);
    public PrinterFaasServingDriver(){initVariables();}
    public synchronized static PrinterFaasServingDriver getInstance() {
        if (driver == null) {
            driver = new PrinterFaasServingDriver();
        }
        return driver;
    }
