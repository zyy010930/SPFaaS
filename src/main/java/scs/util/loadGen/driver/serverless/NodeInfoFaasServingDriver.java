...
public class NodeInfoFaasServingDriver extends AbstractJobDriver{
    /**
     * Singleton code block
     */
    private static NodeInfoFaasServingDriver driver=null;
    private SSHTool tool = new SSHTool("192.168.1.4", "root", "wnlof309b507", StandardCharsets.UTF_8);
    public NodeInfoFaasServingDriver(){initVariables();}
    public synchronized static NodeInfoFaasServingDriver getInstance() {
        if (driver == null) {
            driver = new NodeInfoFaasServingDriver();
        }
        return driver;
    }
