...
public class NsLookUpFaasServingDriver extends AbstractJobDriver {
    /**a
     * Singleton code block
     */
    private static NsLookUpFaasServingDriver driver=null;
    private SSHTool tool = new SSHTool("192.168.1.4", "root", "wnlof309b507", StandardCharsets.UTF_8);
    public NsLookUpFaasServingDriver(){initVariables();}
    public synchronized static NsLookUpFaasServingDriver getInstance() {
        if (driver == null) {
            driver = new NsLookUpFaasServingDriver();
        }
        return driver;
    }
