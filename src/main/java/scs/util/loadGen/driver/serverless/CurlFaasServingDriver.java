...
public class CurlFaasServingDriver extends AbstractJobDriver{
    /**
     * Singleton code block
     */
    private static CurlFaasServingDriver driver=null;
    private SSHTool tool = new SSHTool("192.168.1.4", "root", "wnlof309b507", StandardCharsets.UTF_8);
    public CurlFaasServingDriver(){initVariables();}
    public synchronized static CurlFaasServingDriver getInstance() {
        if (driver == null) {
            driver = new CurlFaasServingDriver();
        }
        return driver;
    }
