...
public class SleepFaasServingDriver extends AbstractJobDriver{
    /**
     * Singleton code block
     */
    private static SleepFaasServingDriver driver=null;
    private SSHTool tool = new SSHTool("192.168.1.4", "root", "wnlof309b507", StandardCharsets.UTF_8);
    public SleepFaasServingDriver(){initVariables();}
    public synchronized static SleepFaasServingDriver getInstance() {
        if (driver == null) {
            driver = new SleepFaasServingDriver();
        }
        return driver;
    }
